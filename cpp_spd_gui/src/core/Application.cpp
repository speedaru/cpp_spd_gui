...
namespace spd::core {
    ...
    void Application::Run(std::chrono::milliseconds frameDelay, std::function<void()> frameCallback) {
        while (m_isRunning) {
            StartRender();
            ...
            if (frameDelay > std::chrono::milliseconds(0)) {
				std::this_thread::sleep_for(frameDelay);
            }

            // per frame callback
            if (frameCallback) {
                std::invoke(frameCallback);
            }
        }
    }
